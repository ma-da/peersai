...
# Simple GBNF grammar for strict JSON output
JSON_GRAMMAR = LlamaGrammar.from_string(r'''
root ::= "{" ws "\"domain\"" ws ":" ws string ws "," ws "\"topics\"" ws ":" ws "[" ws (string ("," ws string)*)? ws "]" ws "}"
string ::= "\"" ([^"] | "\\\"")* "\""
ws ::= [ \t\n]*
''')

MODEL_PATH = "/Volumes/SSK/ai_models/llm_models/Llama/3.1-8B-Instruct/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf"
...
llm = Llama(
    ...
    n_ctx=4096,
    n_threads=8,
    n_gpu_layers=-1,          # ← all layers to Metal if possible
    temperature=0.0,
...
def classify_chunk(text: str) -> dict:
    # Optional: take more intelligent slice (first + last part)
    truncated = (text[:700] + "\n...\n" + text[-700:]) if len(text) > 1400 else text

    domain = classify_domain_fast(truncated)

    prompt = PROMPT_TEMPLATE.format(
        #domains=", ".join(DOMAINS.keys()),   # assuming DOMAINS is dict
        domains=domain,
    ...
    try:
        output = llm(
            ...
            grammar=JSON_GRAMMAR,      # ← enforces valid JSON
            stop=["</s>", "\n\n"]      # safety
        )
        result = output["choices"][0]["text"].strip()
    except Exception as e:
        raise ValueError({"error": str(e), "raw": result if 'result' in locals() else ""})
